...
contract('BNB Staking', async ([alice, bob, admin, dev, minter]) => {
  ...
  it('should block man who in blanklist', async () => {
    ...
    await expectRevert(
      bnbChef.deposit({ from: alice, value: 100 }),
      'in black list'
    )
    ...
    await expectRevert(
      bnbChef.setBlackList(alice, { from: admin }),
      'admin: wut?'
    )
  })
  ...
  it('emergencyRewardWithdraw', async () => {
    await expectRevert(
      bnbChef.emergencyRewardWithdraw(100, { from: alice }),
  ...
  it('setLimitAmount', async () => {
    ...
    await expectRevert(
      bnbChef.deposit({ from: alice, value: 100000000 }),
      'exceed the to'
    )
  })
})

...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
  ...
  it('should not allow non-owner to do operation', async () => {
    ...
    await expectRevert(
      cake.transferOwnership(carol, { from: alice }),
      'Ownable: caller is not the owner'
    )
    await expectRevert(
      cake.transferOwnership(carol, { from: bob }),
      'Ownable: caller is not the owner'
    )
    await expectRevert(
  ...
  it('should also work with MasterChef', async () => {
    ...
    chef = await MasterChef.new(cake.address, syrup.address, dev, '1000', '0', {
      from: alice,
    })
    ...
    await expectRevert(
      chef.add('100', lp1.address, true, { from: alice }),
      'Ownable: caller is not the owner'
    )
